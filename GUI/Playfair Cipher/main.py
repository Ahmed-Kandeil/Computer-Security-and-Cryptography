...
def main(page: ft.Page):
    ...
    def encrypt(e: ControlEvent):
        plain_text_tokens = plain_text_field.value.strip().split()
        key = key_field.value

        encrypter = PlayfairCipher(key=key)
        encrypted_text = []

        for plain_text in plain_text_tokens:
            encrypted_text.append(encrypter.encrypt(plain_text))
    ...
    def decrypt(e: ControlEvent):
        encrypted_text_tokens = cipher_text_field.value.strip().split()
        print(encrypted_text_tokens)
        key = key_field.value

        encrypter = PlayfairCipher(key=key)
        decrypted_text = []

        for encrypted_text in encrypted_text_tokens:
            decrypted_text.append(encrypter.decrypt(encrypted_text))
            print(encrypter.decrypt(encrypted_text))
    ...
    page.add(
        ...
        ft.Row(
            [plain_text_field, cipher_text_field], alignment=ft.MainAxisAlignment.CENTER
        ),
        ft.Row(
